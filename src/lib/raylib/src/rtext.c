    ...
    while ((lineSpacing + lineToRead * (charHeight + lineSpacing)) < image.height)
    {
               ...
               !COLOR_EQUAL((pixels[(lineSpacing + (charHeight + lineSpacing) * lineToRead) * image.width + xPosToRead]), key))
        {
            ...
            while (((xPosToRead + charWidth) < image.width) &&
                   !COLOR_EQUAL(pixels[(lineSpacing + (charHeight + lineSpacing) * lineToRead) * image.width + xPosToRead + charWidth], key))
                charWidth++;
...
Vector2 MeasureTextCodepoints(Font font, const int *codepoints, int length, float fontSize, float spacing)
{
    ...
    if ((font.texture.id == 0) || (codepoints == NULL) || (length == 0))
        return textSize; // Security check
...
const char *TextSubtext(const char *text, int position, int length)
{
    ...
    if (text != NULL)
    {
        ...
        if (position < textLength)
        {
            ...
            if (length > maxLength)
                length = maxLength;
            if (length >= MAX_TEXT_BUFFER_LENGTH)
                length = MAX_TEXT_BUFFER_LENGTH - 1;
    ...
    if (beginIndex > -1)
    {
        ...
        if (endIndex > -1)
        {
            ...
            if (len < (MAX_TEXT_BUFFER_LENGTH - 1))
                strncpy(buffer, text + beginIndex + beginLen, len);
            else
                strncpy(buffer, text + beginIndex + beginLen, MAX_TEXT_BUFFER_LENGTH - 1);
        }
    }
...
char *TextReplace(const char *text, const char *search, const char *replacement)
{
    ...
    if ((text != NULL) && (search != NULL) && (search[0] != '\0'))
    {
        ...
        for (count = 0; (tempPtr = strstr(insertPoint, search)); count++)
            insertPoint = tempPtr + searchLen;
        ...
        if ((textLen + count * (replaceLen - searchLen)) < (MAX_TEXT_BUFFER_LENGTH - 1))
        {
            ...
            TRACELOG(LOG_WARNING, "Text with replacement is longer than internal buffer, use TextReplaceAlloc()");
    }
...
char *TextReplaceAlloc(const char *text, const char *search, const char *replacement)
{
    ...
    if ((text != NULL) && (search != NULL) && (search[0] != '\0'))
    {
        ...
        // Count the number of replacements needed
        insertPoint = (char *)text;
        for (count = 0; (temp = strstr(insertPoint, search)); count++)
    ...
    if ((text != NULL) && (begin != NULL) && (end != NULL))
    {
        ...
        if (beginIndex > -1)
        {
            ...
            if (endIndex > -1)
            {
                ...
                if (replacement != NULL)
                    strncpy(buffer + beginIndex + beginLen, replacement, replaceLen);                      // Copy replacement (if provided)
    ...
    if ((text != NULL) && (begin != NULL) && (end != NULL))
    {
        ...
        if (beginIndex > -1)
        {
            ...
            if (endIndex > -1)
            {
                ...
                if (replacement != NULL)
                    strncpy(result + beginIndex + beginLen, replacement, replaceLen);                      // Copy replacement (if provided)
...
char *TextInsert(const char *text, const char *insert, int position)
{
    ...
    if ((text != NULL) && (insert != NULL))
    {
        ...
        if ((textLen + insertLen) < (MAX_TEXT_BUFFER_LENGTH - 1))
        {
            ...
            TRACELOG(LOG_WARNING, "Text with inserted string is longer than internal buffer, use TextInserExt()");
    }
...
#define MAX_BUFFER_SIZE 256
    ...
    {
        for (int i = 0; i < codepointCount; i++)
            requiredCodepoints[i] = codepoints[i];
    }
                ...
                if (glyphs != NULL)
                {
                    ...
                    for (int x = 0; x < readBytes; x++)
                    {
                        ...
                        for (int bitX = 0; bitX < 4; bitX++)
                        {
                            ...
                            if (pixelX >= glyphs->image.width)
                                break;

                            if ((byte & (8 >> bitX)) > 0)
                                ((unsigned char *)glyphs->image.data)[(pixelY * glyphs->image.width) + pixelX] = 255;
                        }
                    }
                }
                continue;
